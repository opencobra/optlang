...
def load_problem(mps_file, warmup_file):
    ...
    sol_temp_file = tempfile.mktemp(suffix='.bas')
    with open(sol_temp_file, 'w') as tmp_handle:
        f = gzip.open(warmup_file, 'rb')
        tmp_handle.write(f.read())
        f.close()
...
def test_miplib(solutions=SOLUTION, problem_dir=PROBLEMS_DIR):
    with open(solutions, "rU") as fhandler:
        for line in fhandler:
            ...
            print "%s: %s" % (name, sol_type)
            expected_solution = MockupSolution(name, sol_type, f, problem_dir)
            problem_file = os.path.join(problem_dir, "%s.mps.gz" % name)
            warmup_file = os.path.join(problem_dir, "%s.bas.gz" % name)
            try:
                if not os.path.exists(problem_file):
                    download_file("%s.mps.gz" % name, problem_dir)
                if not os.path.exists(warmup_file):
                    download_file("%s.bas.gz" % name, problem_dir)
            except Exception as e:
                raise e

            glpk_problem, model = load_problem(problem_file, warmup_file)
