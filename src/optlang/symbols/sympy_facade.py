...
class UniqueSymbol(sympy.Dummy):
    """
    Present a uniform facade to the ``sympy.Symbol`` class.

    ``Dummy`` is a unique implementation of ``sympy.Symbol`` such that two
    objects with the same name are different. The use of ``__slots__`` is
    propagated.
